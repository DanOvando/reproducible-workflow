...
knitr::include_graphics("https://pbs.twimg.com/media/EjkVI4YWoAAXhxR?format=jpg&name=medium")
```
...
```{r, error = TRUE}
hake <- read.csv(here("home-folder","Place With all the_Data","hake.csv"))
```
...
From a pure reproducibility standpoint this `renv` is great
...
From a practical perspective has some very nice tricks
  ...
  - Every project has its own cache! No more "I don't update any packages because I need the ancient version of some specific package in order for my code to run"
    - Can have `icesDatras` v1.3 in one projects and v1.0 in another
...
Docker won't save you from fundamental hardware problems (e.g. if a given program can only run on Windows)
...
I tend store push small-to-medium data with GitHub. [GitHub Large File Storage](https://docs.github.com/en/free-pro-team@latest/github/managing-large-files/about-git-large-file-storage) is also an option, but costs money 
...
All materials can be accesed (and reproduced!) from github.com/danovando/https://github.com/DanOvando/reproducible-workflow
